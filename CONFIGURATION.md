...
Every `persist.vendor.audio.*` property below has a `ro.vendor.audio.*` counterpart
with the same suffix. The HAL reads the `persist.` variant first and falls back to
the `ro.` variant, then to the built-in default. Use the `ro.` variant in
`PRODUCT_VENDOR_PROPERTIES` to ship a board default that can still be overridden at
runtime with `setprop`.

| Property suffix | Default | Meaning |
| --- | --- | --- |
| `primary.card_name` | *(empty)* | ALSA card name looked up in `/proc/asound/cards` |
| `primary.card` | `0` | ALSA card index, used when `card_name` is unset or not found |
| `primary.device` | `0` | ALSA device index of the primary PCM |
| `mixer.config` | `/vendor/etc/mixer_controls.xml` | Path to the mixer controls config |
| `primary.latency_ms` | `85` | Nominal latency reported for non-MMAP primary streams |
