...
The mixer resolves the ALSA card at initialization, preferring the card name over
the card index, and the `persist.` property over its `ro.` counterpart:

```cpp
int card = ::android::base::GetIntProperty(
